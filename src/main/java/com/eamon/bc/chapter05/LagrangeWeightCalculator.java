 ...
 * @description: *	A basic calculator for Lagrangian weights for a given number of peers in a given field.
 */
                ...
                for (int peerIndex = 0; peerIndex < numberOfPeers; peerIndex++) {
                    ...
                    if (peerIndex != i && activePeers[peerIndex] != null) {
                        ...
                        denominator = denominator.multiply(alphas[peerIndex]
                                .subtract(alphas[i]).mod(field)).mod(field);
                    }
                }
                weights[i] = numerator.multiply(denominator.modInverse(field)).mod(field);
            }
        }
        return weights;
    }
