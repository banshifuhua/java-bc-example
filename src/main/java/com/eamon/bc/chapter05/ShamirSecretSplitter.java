...
/**
 * @author: eamon
 * @date: 2019-01-31 09:06
 * @description: A secret splitter based on Shamir's method.
...
public class ShamirSecretSplitter {
     ...
     */
    public ShamirSecretSplitter(int numberOfPeers, int threshold, BigInteger field, SecureRandom random) {
    ...
    public SplitSecret split(BigInteger secret) {
        ...
        for (int degree = 1; degree < k; degree++) {
            ...
            for (int peerIndex = 0; peerIndex < numberOfPeers; peerIndex++) {
                shares[peerIndex] = shares[peerIndex].add(coefficient.multiply(alphasPow[peerIndex][degree]).mod(order)
                ).mod(order);
            }
        }
    ...
    /**
     * Shamir's original paper actually gives the set [0, fieldSize) as the range // in which coefficients can be chosen, this isn't true for the highest
     * order term as it would have the effect of reducing the order of the
     * polynomial. We guard against this by using the set (0, fieldSize) and // so removing the chance of 0.
     *
     * @param n
     * @param random
     * @return
